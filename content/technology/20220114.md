...
MinIO 同样[支持 DNS Style Bucket](https://github.com/minio/minio/issues/4681)，但默认情况不开启，要开启需要使用环境变量 `MINIO_DOMAIN=domain.com` 来为 MinIO 设置 FQDN。例如你的 Bucket 名称是 assets，则访问方式就是 `assets.domain.com` 。开启了 DNS Style Bucket 模式后就不用在通过 Nginx 层来实现 subdomain 到 /path 的映射了。
