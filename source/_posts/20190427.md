...
![服务端设置管理](/images/article/outline-manager-setting.png)
你可以查看 VPS 信息以及修改服务名称。

![客户端连接管理](/images/article/outline-manager-connections.png)
此时你可添加一个 Key 添加完成后，可以修改 Key 的名字。

![复制 Key](/images/article/outline-manager-copy-key.png)
...
![运行 Outline Client](/images/article/outline-client-running.png)
当 Outline Client 运行后，点击右上角的加号添加一个服务器。

![添加服务器 Key](/images/article/outline-client-add-server.png)
在弹出的输入框中粘贴之前创建 Key 是复制的内容。

![服务器添加完成](/images/article/outline-client-server-ready.png)
如果不出意外的话，你将看到这样的界面。

![建立连接](/images/article/outline-client-connection.png)
点击界面中央的圆圈或者右下角的`连接`按钮，建立 VPN 连接。
...
需要注意的是，截止到发布本文章时，iOS 版并没有在国内的 App Store 上架，如果你是 iOS 设备，需要使用非大陆的 Apple ID 进行登录，并下载 Outline Client。
