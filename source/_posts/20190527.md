...
将如下配置写入 `/etc/consul/conf.d/server.json`，`acl.tokens.master` 是用于访问的凭证，你可以替换成自己的 Token。
...
另外我还提供了一个使用 Docker Compose 方式来启动集群的仓库[https://github.com/betterde/architecture/tree/master/consul](https://github.com/betterde/architecture/tree/master/consul)
