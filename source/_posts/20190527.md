...
toc: true
comments: true
thumbnail: /cover/20190527-consul-cluster.png
tags:
  - Consul
  - Cluster
  - Golang
...
```bash
$ docker network create betterde --subnet 10.0.0.0/24 --gateway 10.0.0.254
```
...
$ docker run -d \
  ...
  consul agent -node=10.0.0.1 -datacenter=betterde -bind=10.0.0.1 \
  -ui -client=0.0.0.0 -server -bootstrap-expect=3

$ docker run -d \
  -h consul-salver-1 \
  --name consul-salver-1 \
  --net=betterde --ip 10.0.0.2 \
  consul agent -node=10.0.0.2 -datacenter=betterde -bind=10.0.0.2 \
  -ui -join=10.0.0.1 -retry-join=10.0.0.1 -server -bootstrap-expect=3

$ docker run -d \
  -h consul-salver-2 \
  --name consul-salver-2 \
  --net=betterde --ip 10.0.0.3 \
  consul agent -node=10.0.0.3 -datacenter=betterde -bind=10.0.0.3 \
  -ui -join=10.0.0.1 -retry-join=10.0.0.1 -server -bootstrap-expect=3
```
...
server {
    listen 80;
    server_name  consul.betterde.com;
...
![Consul Cluster](/images/article/20190527-consul-cluster-in-docker.png)
