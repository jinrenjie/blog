...
toc: true
comments: true
thumbnail: /cover/gitlab-pages.jpg
tags:
  - Pages
  - Gitlab
  - HTTPS
...
```
> Pages 服务的域名如果要开启 HTTPS 的话，一定要使用通配证书，关于如何获取，请参考我的博文[《为你的域名申请统配证书》](https://george.betterde.com/technology/20190117/)

## 验证结果

### 创建项目

访问 `http(s)://example.com/projects/new` 然后和点击 `Create from template` 并点击列表中 `Pages/Plain HTML` 项的 `Use template` 按钮来创建项目。

![从模板创建项目](/images/article/gitlab-projects-new.png)

### 为项目开启 Runner

访问 `http(s)://example.com/YOURNAME/PROJECT_NAME/settings/ci_cd` 展开 Runners 项的配置，查看是否有可用 Runner 如果有，则记录下 Tag，如果没有则参考我的文章[《DevOps 之注册 Gitlab Runner》](https://george.betterde.com/technology/DevOps/20190525/) 进行注册。
...
等待任务执行完成后，则可以访问 `http(s)://example.com/YOURNAME/PROJECT_NAME/pages` 页面，查看 Pages 服务生成的 URL 

![查看项目的 Pages 服务配置](/images/article/gitlab-settings-pages.png)
